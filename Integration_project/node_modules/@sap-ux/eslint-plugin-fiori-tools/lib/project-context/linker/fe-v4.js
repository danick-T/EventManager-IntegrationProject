...
exports.runFeV4Linker = runFeV4Linker;
 ...
 */
function createTable(configurationKey, pathToPage, table) {
    const base = {
        configuration: {
            tableType: {
                configurationPath: [
 ...
 */
function getCreationModeValues(tableType) {
 ...
 */
function runFeV4Linker(context) {
    ...
    for (const [name, target] of Object.entries(routingTargets)) {
        ...
        const contextPath = target.options?.settings?.contextPath ??
        ...
        if (target.name === 'sap.fe.templates.ListReport') {
            ...
            const sections = (0, annotations_1.collectSections)('v4', entity.structuredType, mainService);
            const page = {
                type: 'object-page',
                targetName: name,
 ...
 */
function linkListReport(context, linkedApp, path, name, contextPath, entity, target) {
    ...
    const tables = (0, annotations_1.collectTables)('v4', entityType, mainService);
    const page = {
 ...
 */
function collectTableSections(section, controls, pagePath) {
 ...
 * @param configuration - The routing target configuration
 */
function linkObjectPageSections(page, pathToPage, pageName, sections, configuration) {
    const controls = {};
 ...
 */
function linkObjectPageHeader(page, target) {
 ...
 */
function getEntity(settings, service) {
...
function linkApplicationSettings(context) {
    const config = context.app.manifestObject['sap.fe'] ?? {};
